...
class SFeatureDeductionRuleProcessor {
	...
	def dispatch void processFeatureDeduction(IFeatureContainer container, SFeatureDeduction deductionDefinition,
		SGrabRule rule /*dispatch*/ ) {
		grabFeature(container, deductionDefinition, rule)
	}

	def dispatch void processFeatureDeduction(IFeatureContainer container, SFeatureDeduction deductionDefinition,
		SMorphRule rule /*dispatch*/ ) {
	...
	def DFeature grabFeature(IFeatureContainer container, SFeatureDeduction deductionDefinition, SRenameRule rule) {
		...
		if (source instanceof DFeature) {
			val syntheticFeature = container.addSyntheticFeature(
				if (rule.renameTo !== null) rule.renameTo else source.name, source, deductionDefinition)
			...
			if (deductionDefinition instanceof SQueryDeduction) {
				...
				var explicitParameters = deductionDefinition.parameters.filter [
					! (it instanceof SQueryParameterDeduction || it.synthetic)
				]
				if (! parameterDeductionDefinitions.exists[deductionDefinition instanceof SGrabRule]) {
					...
					val sourceParametersAffectedByRule = parameterDeductionDefinitions.filter [
						deductionRule.source instanceof DQueryParameter
					].map[deductionRule.source as DQueryParameter]
		...
		if (desc.deductionDefinition === null) {
			...
			if (! featureDeductionDefinitions.exists[deductionRule instanceof SGrabRule]) {
				...
				val sourceFeaturesAffectedByRule = featureDeductionDefinitions.filter [
					deductionRule.source instanceof DFeature
				].map[deductionRule.source as DFeature]
	...
	def dispatch void processQueryParameterDeduction(DQuery container, SQueryParameterDeduction deductionDefinition,
		SGrabRule rule) {
	...
	def DQueryParameter grabQueryParameter(DQuery container, SQueryParameterDeduction deductionDefinition,
		SRenameRule rule) {
		val source = rule.source
	...
	def dispatch void processQueryParameterDeduction(SQueryDeduction container,
		SQueryParameterDeduction deductionDefinition, SDitchRule rule) {
