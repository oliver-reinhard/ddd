...
class DmxPrimitiveDescriptor extends AbstractDmxTypeDescriptor<DPrimitive> {
	new(DPrimitive type, boolean collection) {
		// The "?" below is a work-around to suppress an NPE because of a failing proxy resolution during a full project rebuild.
		// Since DmxTypeDescriptors are re-calculate each time, a new descriptor with a resolved proxy will be created at a later time.
