...
class DomDiagramTextProvider extends AbstractDiagramTextProvider {
	...
	def  generateSnapshot(DomSnapshot s) {
		...
		val allReferences = EcoreUtil2.eAllOfType(s, DomField).filter[value instanceof DomDetail]
		val result = '''	
			' snapshot «s.name»
			frame «s.name» <<Snapshot>> {
			«FOR obj:allComplexObjects»«obj.generate»«ENDFOR»
			«FOR obj:allReferences»«obj.generateReference(obj.value as DomDetail)»«ENDFOR»
			}
		'''
		return result
	}
	...
	def String generateReference(DomField f, DomDetail detail) '''
	...
	def String title(DomComplexObject o) {
		...
		val instanceName = switch container {
			DomNamedComplexObject: container.name
			DomField: if (container.ref !== null) container.ref.name else ""
			default: ""
		}
	...
	def  String id(DomComplexObject o) {
		...
		var EObject current = o
		while (current !== null && ! (current instanceof DomSnapshot)) {
			switch current {
				DomField: if (current.ref !== null) { b.append(current.ref.name) b.append("_")}
				DomObject: if (current.name !== null) { b.append(current.name) b.append("_")}
				default: b.append("")
			}
