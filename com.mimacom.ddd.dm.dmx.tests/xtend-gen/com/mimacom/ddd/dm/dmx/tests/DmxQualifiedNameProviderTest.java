...
public class DmxQualifiedNameProviderTest {
  ...
  private final DimFactory DIM = DimFactory.eINSTANCE;
  ...
  public void testWithNamespace() {
    ...
    final DomainInformationModel model = this.DIM.createDomainInformationModel();
    model.setName("model");
    namespace.setModel(model);
    final DimPrimitive primitive1 = this.DIM.createDimPrimitive();
    primitive1.setName("primitive1");
    model.getTypes().add(primitive1);
    final DimAggregate aggregate = this.DIM.createDimAggregate();
    aggregate.setName("aggr");
    model.getAggregates().add(aggregate);
    final DimEntityType root = this.DIM.createDimEntityType();
    root.setRoot(true);
    root.setName("root");
    aggregate.getTypes().add(root);
    final DimAttribute attr1 = this.DIM.createDimAttribute();
    attr1.setName("attr1");
    root.getFeatures().add(attr1);
    final DimQuery query1 = this.DIM.createDimQuery();
    query1.setName("query1");
    root.getFeatures().add(query1);
    final DimQueryParameter queryParam1 = this.DIM.createDimQueryParameter();
    queryParam1.setName("queryParam1");
    query1.getParameters().add(queryParam1);
    final DimEntityType entity = this.DIM.createDimEntityType();
    entity.setName("entity");
    aggregate.getTypes().add(entity);
    final DimAttribute attr2 = this.DIM.createDimAttribute();
    attr2.setName("attr2");
    entity.getFeatures().add(attr2);
    entity.getFeatures().add(attr2);
    final DimQuery query2 = this.DIM.createDimQuery();
    query2.setName("query1");
    entity.getFeatures().add(query2);
    final DimQueryParameter queryParam2 = this.DIM.createDimQueryParameter();
    queryParam2.setName("queryParam1");
    query2.getParameters().add(queryParam2);
    final DimPrimitive primitive2 = this.DIM.createDimPrimitive();
    primitive2.setName("primitive2");
    aggregate.getTypes().add(primitive2);
    final DimEnumeration enumeration = this.DIM.createDimEnumeration();
    enumeration.setName("enumeration");
    aggregate.getTypes().add(enumeration);
    final DimLiteral literal = this.DIM.createDimLiteral();
